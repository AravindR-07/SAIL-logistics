...
import {
    ...
    DollarSign, BarChart3, TrendingUp, RefreshCw, Zap
} from 'lucide-react';
import {
...
// --- Mock Calculation Helper (Legacy/Fallback) ---
...
const Card = ({ children, className = '', onClick }: { children: React.ReactNode; className?: string; onClick?: () => void }) => (
    <motion.div
        ...
        onClick={onClick}
        className={`bg-white backdrop-blur-xl border border-black/5 rounded-2xl p-6 shadow-card overflow-hidden relative group ${className} ${onClick ? 'cursor-pointer hover:border-blue-600/50 transition-colors' : ''}`}
    >
        {children}
    </motion.div>
);
...
const WhatIfAnalysis = () => {
    ...
    // Auto-calculates purely for UI responsiveness before the actual run
    // const [results, setResults] = useState(calculateImpactMock(DEFAULT_PARAMS, activeId));
    ...
    return (
        <div className="min-h-screen p-6 text-black font-sans relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    {/* Tabs */}
                    <div className="flex bg-white p-1 rounded-lg w-fit border border-white/10">
                        {(['Vessel', 'Port', 'Rail', 'Plant'] as SimulationLevel[]).map(level => (
                            <button
                                ...
                                className={`px-6 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === level
                                    ...
                                    : 'text-text-tertiary hover:text-black hover:bg-white/5'
                                    }`}
                            >
                                {level} Level
                            </button>
            ...
            {/* ChatBot Pop-over - Moved outside to escape overflow-hidden */}

...
const RailwayDispatch = () => {
  const { user } = useAuth();
  const canEdit = ['admin', 'corporate_logistics', 'railway_officer'].includes(user?.role || '');
  ...
  const handleExportPDF = async () => {
    setIsExporting(true)
    const element = document.getElementById('railway-content')
    if (!element) {
    ...
    try {
      const canvas = await html2canvas(element, { scale: 2, logging: false, windowWidth: 1600 })
  ...
  return (
    <div className="w-full" id="railway-content">
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-semibold text-black mb-2">Railway Dispatch Planning</h1>
